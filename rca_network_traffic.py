...
========================================================
  ...
  Dataset  : CICIDS 2017 (full 8-file set)
  Attacks  : BENIGN, Brute Force, DoS Hulk/GoldenEye/Slowloris/
             Slowhttptest, Heartbleed, Web Attack (Brute Force/
             XSS/SQL Injection), Infiltration, Bot, PortScan, DDoS
...
if len(CSV_FILES) == 0:
    raise FileNotFoundError("No CSV files found in uploads folder.")

OUTPUT_DIR       = "rca_outputs"   # All plots + reports saved here
SAMPLE_SIZE      = 200             # Flows to explain via SHAP (keep <= 50 for speed)
TEST_SIZE        = 0.2
RANDOM_STATE     = 42
TOP_FEATURES     = 10              # Features shown in SHAP plots
TOP_RCA          = 3               # Top features per flow in RCA report
MIN_SAMPLES      = 20              # Classes with fewer rows than this are dropped
BENIGN_KEEP_FRAC = 0.15            # Fraction of BENIGN rows to keep per file (downsampling)
...
for filepath in CSV_FILES:
    ...
    # ---- Label cleanup: strip whitespace + normalize dash characters ----
    df["Label"] = df["Label"].str.strip()
    df["Label"] = df["Label"].str.replace("\u2013", "-", regex=False)  # en-dash -> hyphen
    df["Label"] = df["Label"].str.replace("\u2014", "-", regex=False)  # em-dash -> hyphen
    df["Label"] = df["Label"].str.replace(r"\s+", " ", regex=True)     # collapse repeated spaces

    # ---- Downsample BENIGN rows so they don't drown out attack traffic ----
    benign_mask = df["Label"] == "BENIGN"
    if benign_mask.any() and BENIGN_KEEP_FRAC < 1.0:
        df_benign = df[benign_mask].sample(frac=BENIGN_KEEP_FRAC, random_state=RANDOM_STATE)
        df_attack = df[~benign_mask]
        df = pd.concat([df_benign, df_attack], ignore_index=True)

    print(f"  Loaded: {filepath} -> {df.shape[0]:,} rows (after BENIGN downsample)")
...
# ---- Drop ultra-rare classes that break stratified split / SMOTE ----
label_counts = data["Label"].value_counts()
rare_labels = label_counts[label_counts < MIN_SAMPLES].index.tolist()
if rare_labels:
    print(f"  Dropping ultra-rare classes (<{MIN_SAMPLES} samples): {rare_labels}")
    data = data[~data["Label"].isin(rare_labels)]

print(f"  Shape after cleaning : {data.shape}")
print(f"  Remaining classes    : {sorted(data['Label'].unique().tolist())}")
...
for cls, cnt in zip(*np.unique(y_train, return_counts=True)):
    print(f"    {le.inverse_transform([cls])[0]:<25} : {cnt:,}")

# k_neighbors must be < smallest class size in the training split, or SMOTE errors out.
# With many more classes now (some naturally rarer), pick k adaptively instead of a fixed 3.
min_class_count = pd.Series(y_train).value_counts().min()
safe_k = max(1, min(3, min_class_count - 1))
print(f"\n  Smallest training class has {min_class_count} samples -> using k_neighbors={safe_k}")

# Limit how much SMOTE oversamples to avoid running out of RAM
...
for cls, count in class_counts.items():
    if count < MAX_SAMPLES_PER_CLASS:
        sampling_strategy[cls] = MAX_SAMPLES_PER_CLASS
...
fig, ax = plt.subplots(figsize=(max(6, n_classes * 0.9), max(5, n_classes * 0.8)))
sns.heatmap(
...
# Randomly choose flows from the entire test set
sample_indices = np.random.choice(
    len(X_test_sel),
    size=min(SAMPLE_SIZE, len(X_test_sel)),
    replace=False
)
...
for attack in le.classes_:
    ...
    plt.tight_layout()
    safe_name = attack.replace(" ", "_").replace("/", "_").lower()
...
for i in range(len(X_sample)):
    ...
    print(f"\n  Flow {i+1:02d} | Predicted: {label_pred:<25} | Actual: {label_actual:<25} | Severity: {severity}/100")
    ...
    for rank, (feat, importance) in enumerate(top_features.items(), start=1):
        ...
        direction  = "Up (High)" if feature_shap[feat] > 0 else "Down (Low)"
...
# Dynamic color palette so this scales past 3 classes without collapsing
# every "unknown" label into the same fallback blue.
unique_labels = rca_df["Predicted_Label"].unique()
palette = plt.colormaps["tab20"].resampled(max(len(unique_labels), 1))
...
for label in unique_labels:
    ...
    ax.scatter(subset.index, subset.values, label=label, color=colors[label], s=80, zorder=3)
...
if alerts:
    ...
    for a in alerts:
        print(f"    Flow {a['flow']:02d} | {a['attack']:<25} | Severity {a['severity']:3d}/100 | "
